Public Class Report_Class
    ...
    Public Function GenerateCashBalance() As List(Of String)
        ...
        Dim cmd = db.cmd
        cmd.Connection = db.conn
        cmd.CommandText = "SET datestyle = dmy;SELECT round(SUM(amount)::DECIMAL, 2)::TEXT as total_cashout, (SELECT SUM(added_amount) as total_cashin FROM funds_transactions WHERE transaction_date >= '" + From_Date + "' AND transaction_date <= '" + To_Date + "') FROM expenses WHERE date >= '" + From_Date + "' AND date <= '" + To_Date + "'"
        Dim dr = db.dr
